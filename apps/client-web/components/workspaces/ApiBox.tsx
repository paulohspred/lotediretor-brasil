...
export async function getJson<T=any>(url:string):Promise<T>{const r=await fetch(url,{credentials:'include',cache:'no-store'});const x=await r.json().catch(()=>null);if(!r.ok)throw new Error(errorMessage(x,r.status));return x;}
...
export async function postJson<T=any>(url:string,body:any,options?:{idempotent?:boolean;retries?:number}):Promise<T>{
  ...
  for(let attempt=0;;attempt++){
    ...
    try{const r=await fetch(url,{method:'POST',headers,credentials:'include',body:JSON.stringify(body)});const x=await r.json().catch(()=>null);if(!r.ok){if(attempt<retries&&[502,503,504].includes(r.status)){await new Promise(res=>setTimeout(res,400*(attempt+1)));continue;}throw new Error(errorMessage(x,r.status));}return x;}
    catch(e){if(attempt<retries){await new Promise(res=>setTimeout(res,400*(attempt+1)));continue;}throw e;}
  }
}
export async function uploadFile<T=any>(file:File):Promise<T>{const form=new FormData();form.append('file',file);const r=await fetch('/api/v1/files/upload',{method:'POST',credentials:'include',body:form});const x=await r.json().catch(()=>null);if(!r.ok)throw new Error(errorMessage(x,r.status));return x;}

...
function boundedPayload(args:any[],kwargs:any){
  const raw=JSON.stringify({args,kwargs});
  if(Buffer.byteLength(raw,'utf8')>1024*1024)throw new HttpException('aitec_job_payload_too_large',413);
}

@Controller('api/v1/aitec')
export class AitecJobController{
  constructor(private readonly auth:AuthService){}
  private async session(req:any){
    const s:any=await this.auth.get(req.cookies?.ld_session);
  ...
  async create(@Req() req:any,@Param('projectId') projectId:string,@Body() body:any){
    ...
    const idem=await withIdempotency(pool,s.organizationId,'aitec.job',key,async c=>{
      const project=await c.query(`select id,status from aitec.project where id=$1 and tenant_id=$2`,[projectId,s.organizationId]);
    ...
    return{...idem.value,idempotency:{key,replayed:idem.replayed,contract:'aitec.job'}};
  }
  ...
  async status(@Req() req:any,@Param('id') id:string){
    ...
    return tenantTx(pool,s.organizationId,async c=>{
      const r=await c.query(`select id,project_id,constraint_snapshot_id,operation,status,attempts,execution_context,solver_version,classification,professional_review_required,engine_response,error,created_by,created_at,started_at,completed_at
        from aitec.job where id=$1 and tenant_id=$2`,[id,s.organizationId]);

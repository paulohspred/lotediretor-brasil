...
export async function withIdempotency<T>(pool:Pool,tenantId:string,scope:string,key:string|undefined,fn:(c:PoolClient)=>Promise<T>):Promise<IdempotentResult<T>>{
  ...
  try{
    ...
    // Acquire the key atomically. An expired row is reclaimed in-place under the
    // unique-key lock, avoiding the delete/insert race and the permanent 409 caused
    // by an expired row still occupying the primary key.
    const acquired=await c.query(`insert into api.idempotency_key as idem(tenant_id,scope,idempotency_key,status,expires_at)
      values($1,$2,$3,'IN_PROGRESS',now()+interval '24 hours')
      on conflict(tenant_id,scope,idempotency_key) do update
      set status='IN_PROGRESS',response=null,completed_at=null,created_at=now(),expires_at=now()+interval '24 hours'
      where idem.expires_at<=now()
      returning idempotency_key`,[tenantId,scope,key]);

...
export class PlatformMetricsController{
  ...
  async metrics(@Res() res:any){
    ...
    const body=[
      ...
      `lotediretor_service_up{service="platform-api"} ${db}`,
      '# HELP lotediretor_process_uptime_seconds Process uptime in seconds.',
      '# TYPE lotediretor_process_uptime_seconds gauge',
      `lotediretor_process_uptime_seconds{service="platform-api"} ${uptime.toFixed(3)}`,
      '# HELP lotediretor_build_info Static build information.',
      '# TYPE lotediretor_build_info gauge',
      'lotediretor_build_info{service="platform-api",version="19.0.0-rc.3"} 1',
      renderHttpMetrics('platform-api'),
      ''
    ].join('\n');

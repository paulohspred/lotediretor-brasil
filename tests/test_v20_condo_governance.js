...
const ops=fs.readFileSync(path.join(root,'services/platform-api/src/condo/condo-v20-ops.controller.ts'),'utf8');for(const token of ['defenses/:defenseId/review','secret_ref','secrets are referenced, never returned','recipient_ref_hash','legal-monitor-events/:eventId/review'])assert(ops.includes(token),token);

...
paths={'unit':'unit_solver.py','terrain':'terrain_solver.py','building':'building_solver.py','basement':'basement_parking_solver.py','room':'room_solver.py','road':'road_engineering_solver.py','environment':'environment_solver.py','finance':'finance_solver.py'}
...
'finance':["FINANCE_SOLVER_VERSION = 'aitec-finance-v20.1'",'calculate_project_finance','market_snapshot_id','cost_snapshot_id','MIN_PROJECT_MARGIN','MAX_TOTAL_COST','pareto_metrics','_irr_monthly','_npv']}
for name,tokens in checks.items():
    for token in tokens:assert token in files[name],token
for name,tokens in {'basement':['does not infer local parking'],'room':['does not infer local'],'road':['does not infer local'],'environment':['not irradiance or energy yield','UNVERIFIED'],'finance':['never invents unit prices','não inventa dados de mercado nem custo','Não substitui orçamento executivo']}.items():
    for token in tokens:assert token in files[name],token
assert 'COPY services/aitec-engine/app ./app' in (root/'services/aitec-engine/Dockerfile').read_text()
for test in ['tests/test_v20_aitec_unit_solver.py','tests/test_v20_aitec_terrain_tin.py','tests/test_v20_aitec_building_solver.py','tests/test_v20_aitec_basement_parking.py','tests/test_v20_aitec_room_graph.py','tests/test_v20_aitec_road_engineering.py','tests/test_v20_aitec_environment.py','tests/test_v20_aitec_finance.py']:subprocess.run(['python',test],check=True)
print('v20 A.I TEC finance contracts OK')

...
sql='\n'.join(p.read_text() for p in required[:2])
for token in [
...
controller=required[2].read_text()
for token in [
...
auth=required[4].read_text();assert 'async logoutUser(userId:string)' in auth
module=required[3].read_text();assert 'PrivacyV20Controller' in module
app=required[5].read_text();assert 'PrivacyModule' in app

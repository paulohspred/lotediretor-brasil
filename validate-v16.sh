...
JS
bash -n $(find ops -name '*.sh' -type f | sort) validate-v16.sh
python - <<'PY'
import json
from pathlib import Path
bad=[]
for p in [Path('package.json'),*Path('apps').glob('*/package.json'),*Path('services').glob('*/package.json'),*Path('packages').glob('*/package.json')]:
 ...
 if d.get('version')!='16.0.0':bad.append(f'{p}:{d.get("version")}')
if bad:raise SystemExit('VERSION DRIFT '+repr(bad))
assert "RELEASE = 'v16'" in Path('services/platform-api/src/version.ts').read_text()
assert "RELEASE='v16'" in Path('services/control-api/src/version.ts').read_text()
print('Current-facing package versions OK')
PY
echo 'v16 validation OK'

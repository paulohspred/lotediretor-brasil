...
bash -n $(find ops -name '*.sh' -type f | sort) validate-v19-rc.sh validate-v19.sh
...
assert "RELEASE = 'v19-rc1'" in Path('services/platform-api/src/version.ts').read_text()
assert "RELEASE='v19-rc1'" in Path('services/control-api/src/version.ts').read_text()
for p in ('services/platform-api/src/v8.metrics.controller.ts','services/control-api/src/v8.metrics.controller.ts'):
    assert 'version="19.0.0-rc.1"' in Path(p).read_text()
...
print('v19-rc domain topology contracts OK')
PY

python - <<'PY'
from pathlib import Path
ci=Path('.github/workflows/ci.yml').read_text();gate=Path('ops/release/gate.sh').read_text()
assert './validate-v19-rc.sh' in ci and './validate-v19-rc.sh' in gate
for old in ('./validate-v13.sh','./validate-v18.sh','./validate-v19-beta.sh'):

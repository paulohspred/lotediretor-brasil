...
bash -n $(find ops -name '*.sh' -type f | sort) validate-v19-rc.sh validate-v19-rc2.sh validate-v19.sh
...
assert "RELEASE = 'v19-rc2'" in Path('services/platform-api/src/version.ts').read_text()
assert "RELEASE='v19-rc2'" in Path('services/control-api/src/version.ts').read_text()
for p in ('services/platform-api/src/v8.metrics.controller.ts','services/control-api/src/v8.metrics.controller.ts'):
    assert 'version="19.0.0-rc.2"' in Path(p).read_text()
...
for x in ['aitec/projects/:id/solutions/generate','aitec/projects/:id/solutions','aitec/solutions/:solutionId','aitec.solutions.generated']:assert x in api,x
print('v19-rc2 domain topology contracts OK')
PY

python - <<'PY'
from pathlib import Path
ci=Path('.github/workflows/ci.yml').read_text();gate=Path('ops/release/gate.sh').read_text()
assert './validate-v19-rc2.sh' in ci and './validate-v19-rc2.sh' in gate
for old in ('./validate-v13.sh','./validate-v18.sh','./validate-v19-beta.sh','./validate-v19-rc.sh'):
